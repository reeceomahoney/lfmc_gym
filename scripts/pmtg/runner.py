...
from raisim_gym_torch.env.bin.pmtg import NormalSampler, RaisimGymEnv
...
def main():
    ...
    _actor_critic_module = modules.get_actor_critic_module_from_config(
        cfg, env, NormalSampler, device
    )
    actor_critic_module_eval = modules.get_actor_critic_module_from_config(
        cfg, env, NormalSampler, device
    )
    ...
    rl_gamma = np.exp(
        ...
        * np.log(0.5)
        / cfg["algorithm"]["gamma_half_life_duration"]
    )
    learning_rate_decay_gamma = np.exp(
        np.log(
            cfg["algorithm"]["learning_rate"]["final"]
            / cfg["algorithm"]["learning_rate"]["initial"]
        )
        / cfg["algorithm"]["learning_rate"]["decay_steps"]
    )  # x_t = x_0 * gamma ^ t

    if cfg["algorithm"]["learning_rate"]["mode"] == "constant":
        learning_rate_decay_gamma = 1.0
    ...
    if mode == "retrain":
        load_param(
            weight_path, env, ppo.actor_critic_module, ppo.optimizer, saver.data_dir
        )
    ...
    for update in range(20500):
        ...
        if update % cfg["environment"]["save_every_n"] == 0:
            ...
            ppo.actor_critic_module.save_parameters(
                saver.data_dir + "/full_" + str(update) + ".pt", parameters_save_dict
            )
        ...
        if update % cfg["environment"]["eval_every_n"] == 0:
            if cfg["environment"]["render"]:
                ...
                actor_critic_module_eval.load_parameters(
                ...
                if cfg["record_video"]:
                    env.start_video_recording(
                        saver.data_dir + "/policy_" + str(update) + ".mp4"
                    )

                for step in range(
                    math.floor(
                        cfg["environment"]["max_time"]
                        / cfg["environment"]["control_dt"]
                    )
                ):
                    with torch.no_grad():
                        ...
                        action_ll = (
                            actor_critic_module_eval.generate_action(
                        ...
                        frame_end = time.time()
                        wait_time = cfg["environment"]["control_dt"] - (
        ...
        if not visualizable_iteration:
            ...
            ppo.actor_critic_module.distribution.enforce_minimum_std(
                (torch.ones(16) * 0.2).to(device)
            )
