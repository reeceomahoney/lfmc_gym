...
class RewardLogger:
    ...
    def log_to_wandb(self, it):
        # Compute Episodic Mean and Standard Deviation of Individual Reward Terms
        for term in self._reward_terms:
            self._reward_mean_dict[term] = np.mean(
                np.array(self._reward_storage_dict[term])
            )
            self._reward_std_dict[term] = np.std(
                np.array(self._reward_storage_dict[term])
            )

            # if self._episodic_reset_count > 0:
            #     self._reward_mean_of_means_dict[term] = np.mean(
            #         np.array(self._reward_mean_moving_storage_dict[term])
            #     )
            #     self._reward_std_of_means_dict[term] = np.std(
            #         np.array(self._reward_mean_moving_storage_dict[term])
            #     )
            #
            #     self._reward_mean_of_stds_dict[term] = np.mean(
            #         np.array(self._reward_std_moving_storage_dict[term])
            #     )
            #     self._reward_std_of_stds_dict[term] = np.std(
            #         np.array(self._reward_std_moving_storage_dict[term])
            #     )
        ...
        # if self._episodic_reset_count > 0:
        #     writer.add_scalars(
        #         "Rewards/Windowed/Means/mean", self._reward_mean_of_means_dict, it
        #     )
        #     writer.add_scalars(
        #         "Rewards/Windowed/Means/std", self._reward_std_of_means_dict, it
        #     )
        #
        #     writer.add_scalars(
        #         "Rewards/Windowed/Stds/mean", self._reward_mean_of_stds_dict, it
        #     )
        #     writer.add_scalars(
        #         "Rewards/Windowed/Stds/std", self._reward_std_of_stds_dict, it
        #     )
    ...
    def episodic_reset(self):
        ...
        for term in self._reward_terms:
            ...
            self._reward_mean_moving_storage_dict[term].append(
                self._reward_mean_dict[term]
            )
            self._reward_std_moving_storage_dict[term].append(
                self._reward_std_dict[term]
            )
    ...
    def reset(self):
        for term in self._reward_terms:
            ...
            self._reward_mean_moving_storage_dict[term] = deque(
                maxlen=self._episodes_window
            )
            self._reward_std_moving_storage_dict[term] = deque(
                maxlen=self._episodes_window
            )
